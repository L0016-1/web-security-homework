...
本项目包含一个 **原始漏洞版本** 和一个 **安全加固版本** 的 Flask 用户管理系统。原始版本存在 **24 项安全漏洞**，涵盖 OWASP Top 10 中的 7 个类别。安全加固版本对所有漏洞进行了系统性修复，并新增了用户注册、登录历史、密码修改等功能。
...
├── original/                     # 原始漏洞版本
│   ├── app.py                    # Flask 主程序（含 20 项漏洞）
│   ├── templates/
│   │   ├── base.html             # 基础模板
│   │   ├── index.html            # 首页（密码明文展示 + 搜索功能）
│   │   ├── login.html            # 登录页（HTML 注释泄露凭证）
│   │   ├── register.html         # 注册页（SQL 注入 + 无输入校验）
│   │   └── upload.html           # 上传页（任意文件上传 + 路径穿越）
│   └── static/
│       ├── css/
│       │   └── style.css         # 样式文件
│       └── uploads/              # 上传文件目录（无类型限制）
├── patched/                      # 安全加固版本
...
| 16 | **SQL 注入（注册接口）** | 🔴 严重 | A03:2021 注入 | `app.py` `/register` |
| 17 | **SQL 注入（搜索接口）** | 🔴 严重 | A03:2021 注入 | `app.py` `/search` |
| 18 | **搜索接口越权访问** | 🟠 高危 | A01:2021 访问控制失效 | `app.py` `/search` |
| 19 | **注册无密码复杂度校验** | 🟡 中危 | A07:2021 身份认证失败 | `app.py` `/register` |
| 20 | **注册/搜索无输入校验** | 🟡 中危 | A03:2021 注入 | `app.py` |
| 21 | **任意文件上传（无类型检查）** | 🔴 严重 | A04:2021 不安全设计 | `app.py` `/upload` |
| 22 | **路径穿越（原始文件名未过滤）** | 🔴 严重 | A01:2021 访问控制失效 | `app.py` `/upload` |
| 23 | **存储型 XSS（可上传 HTML 直接执行）** | 🟠 高危 | A03:2021 注入 | `app.py` `/upload` |
| 24 | **上传接口无 CSRF 防护** | 🟡 中危 | A01:2021 访问控制失效 | `upload.html` |
